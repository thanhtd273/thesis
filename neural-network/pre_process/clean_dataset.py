...
def create_benign_dataset(
    ...
    p.starmap(
        ...
        product(
            [data], [dates], [begin_date], [time_step], nodes, [benign_data], [benign_data_rows],
        ),
    )
    p.close()
    p.join()
    ...
    # Tạo đối tượng phân phỗi xác suất
    ...
    # Tạo n giá trị ngẫu nhiên từ phân phối xác suất trên, n = số hàng của benign_data
...
def main_generate_benign_data(time_step):
    ...
    # Clean data: filter original data, only keep data between begin_date and end_date
    ...
    # Calculate Cauchy distribution of N_BaIoT
    ...
    for index in range(CONFIG.NUM_GROUPS):
        ...
        benign_data_output_path = (
            f"{CONFIG.OUTPUT_DIRECTORY}/pre_process/Output/group_{str(index)}/benign_data/benign_data_{str(begin_date)}_{str(end_date)}_time_step_{str(time_step)}_num_ids_{str(num_nodes_per_group)}.csv"
        )
        prepare_output_directory(benign_data_output_path)
        create_benign_dataset(
            original_data, begin_date, end_date, time_step, selected_nodes, benign_packet_cauchy, benign_data_output_path,
        )
