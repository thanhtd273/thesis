...
def generate_attack(benign_data, attack_begin_date, attack_end_date, attacked_ratio_nodes, attack_duration, attack_start_time, attack_packet_cauchy, attack_parameter, num_days, output_path, data_type):
    ...
    random.seed()
    data_selected_nodes = benign_data.loc[(benign_data["TIME"] >= attack_begin_date) & (benign_data["TIME"] < attack_end_date)]
    ...
    benign_data = benign_data[["BEGIN_DATE", "END_DATE", "NUM_NODES", "ATTACK_RATIO", "ATTACK_START_TIME", "ATTACK_DURATION", "ATTACK_PARAMETER", "NODE", "LAT", "LNG", "TIME", "TIME_HOUR", "ACTIVE", "PACKET", "ATTACKED"]]

    output_path += f"attacked_data_{data_type}_{str(attack_begin_date)}_{str(attack_end_date)}_ratio_{str(attacked_ratio_nodes)}_start_time_{str(attack_start_time)}_duration_{str(attack_duration)}_k_{str(attack_parameter)}.csv"
...
def main_generate_attack(
    ...
    output_path = f"{dataset_directory_path}group_{str(group_number)}/attacked_data/{data_type}/"
    ...
    benign_data = benign_data.loc[(benign_data["TIME"] >= slice_benign_data_start) & (benign_data["TIME"] < slice_benign_data_end)]
    ...
    output_path_benign = f"{output_path}attacked_data_{data_type}_{str(attack_begin_date)}_{str(attack_end_date)}_ratio_0_start_time_{str(attack_begin_date)}duration_0_k_0.csv"
    ...
    benign_packet_path = (
        f"{CONFIG.DATASET_DIRECTORY}N_BaIoT/SimpleHome_XCS7_1003_WHT_Security_Camera_benign_aggregation_Source-IP_time_window_10-sec_stat_Number.csv"
    )
    ...
    for k in k_list:
        ...
        p.starmap(
            generate_attack,
            product([benign_data], [attack_begin_date], [attack_end_date], attacked_ratio_nodes_list, attack_duration_list, attack_start_times_list, [packet_cauchy], [k], [num_days], [output_path], [data_type]),
        )
        p.close()
        p.join()
...
def main():
    ...
    dataset_directory_path = f"{CONFIG.OUTPUT_DIRECTORY}pre_process/Output/"
    if len(sys.argv) > 1:
        print(sys.argv)
    ...
    for group_number in range(0, 1):
        main_generate_attack(
            dataset_directory_path, group_number, "train", num_train_days, num_validation_days, num_test_days, k_list, attacked_ratio_nodes_list, attack_duration_list, attack_start_times_list, time_step, num_nodes,
        )
        print("generate train data done.")
        main_generate_attack(
            dataset_directory_path, group_number, "validation", num_train_days, num_validation_days, num_test_days, k_list, attacked_ratio_nodes_list, attack_duration_list, attack_start_times_list, time_step, num_nodes,
        )
        print("generate validation data done.")
        main_generate_attack(
            dataset_directory_path, group_number, "test", num_train_days, num_validation_days, num_test_days, k_list, attacked_ratio_nodes_list, attack_duration_list, attack_start_times_list, time_step, num_nodes,
        )
