...
def combine_data(num_days, input_path, output_path):
    ...
    columns = ["BEGIN_DATE", "END_DATE", "NUM_NODES", "ATTACK_RATIO", "ATTACK_START_TIME", "ATTACK_DURATION", "ATTACK_PARAMETER", "NODE", "LAT", "LNG", "TIME", "TIME_HOUR", "ACTIVE", "PACKET", "ATTACKED"]
    for node in nodes:
    ...
    p = Pool(28)
    p.starmap(add_other_nodes_packets, product(all_files, [num_days], [combined_data_rows]))
    p.close()
    p.join()
...
def main_combine_data(group_number, data_type, num_days):
    ...
    """
    if data_type == "train":
        input_path = f"{CONFIG.OUTPUT_DIRECTORY}pre_process/Output/group_{str(group_number)}/attacked_data/train/*.csv"
        output_path = f"{CONFIG.OUTPUT_DIRECTORY}pre_process/Output/group_{str(group_number)}/train_data/train_data.csv"
    elif data_type == "validation":
        input_path = f"{CONFIG.OUTPUT_DIRECTORY}pre_process/Output/group_{str(group_number)}/attacked_data/validation/*.csv"
        output_path = f"{CONFIG.OUTPUT_DIRECTORY}pre_process/Output/group_{str(group_number)}/validation_data/validation_data.csv"
    elif data_type == "test":
        input_path = f"{CONFIG.OUTPUT_DIRECTORY}pre_process/Output/group_{str(group_number)}/attacked_data/test/*.csv"
        output_path = f"{CONFIG.OUTPUT_DIRECTORY}pre_process/Output/group_{str(group_number)}/test_data/test_data.csv"
